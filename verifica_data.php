	...
	function verifica_data(){
	    ...
	    foreach ($data as $key => $entry) {
	        ...
	        if ($dia_atual == $dia && $mes_atual == $mes && $ano_atual == $ano){
	        	$controle = 1;
	            $dados = file_get_contents('vencidas.json');
	            ...
	            $arquivo = fopen("vencidas.json", "w");
	            fwrite($arquivo, $dados_json);
	            fclose($arquivo);

	            $dados2 = file_get_contents('doacoes_finalizadas.json');
	            ...
	            $arquivo2 = fopen("doacoes_finalizadas.json", "w");
